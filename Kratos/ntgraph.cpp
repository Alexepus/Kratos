...
void CNTGraph::PlotData()
{
	...
	for(pReg=CDxpsRegion::GetFirst(); pReg!=0; pReg=pReg->GetNext(pReg))
	{
		if(pReg->ID!=0)
			AddElement();
		SetElementPointSymbol(2);
		SetElementPointColor(theApp.m_pMainFrame->m_pDxpsDlg->m_ColorIndex[CDxpsRegion::GetRegByN(pReg->ID)->Parameters.ColorIndex]);
		SetElementIdentify(TRUE);
		SetElementLineColor(theApp.m_pMainFrame->m_pDxpsDlg->m_ColorIndex[CDxpsRegion::GetRegByN(pReg->ID)->Parameters.ColorIndex]);
		//AddAnnotation();
		//str.Format("D%i", pReg->ID+1);
		//SetAnnoLabelCaption((LPCSTR)str);
		//SetAnnoLabelX(0.00001);
		//SetAnnoLabelY(1.9-0.1*pReg->ID);
		SetElementName((LPCSTR)str);
		SetAnnoLabelColor(theApp.m_pMainFrame->m_pDxpsDlg->m_ColorIndex[CDxpsRegion::GetRegByN(pReg->ID)->Parameters.ColorIndex]);
	...
	{
		A=*i;
		if(WasFirstSeriesPoint[i->RegionN]==FALSE)
		{
			WasFirstSeriesPoint[i->RegionN]=TRUE;
			MaxSeriesValue[i->RegionN]=i->Freq*2;
		}
		else if(MaxSeriesValue[i->RegionN]<i->Freq)
			MaxSeriesValue[i->RegionN]=i->Freq;
	}

	for (i = CDxpsRegion::OutData.begin(); i != CDxpsRegion::OutData.end(); i++)
	{
		if(MaxSeriesValue[i->RegionN]>0)
		{
			PlotXY(i->Time/24./60./60., i->Freq*100/MaxSeriesValue[i->RegionN], i->RegionN);
		}
		else
		{
			PlotXY(i->Time/24./60./60., 0, i->RegionN);
		}
		LastSeriesIndex[i->RegionN]++;

	}
	for(pReg=CDxpsRegion::GetFirst(); pReg!=0; pReg=pReg->GetNext(pReg))
	{
		SetElement(pReg->ID);
		str.Format("D%i: 100%% = %g Hz", pReg->ID+1, MaxSeriesValue[pReg->ID]);
		SetElementName((LPCSTR)str);
	}

}

void CNTGraph::PlotNewSegment(DxpsOutData &OutData)
{
	CString str;
	if(WasFirstSeriesPoint[OutData.RegionN]==FALSE)
	{
		WasFirstSeriesPoint[OutData.RegionN]=TRUE;
		MaxSeriesValue[OutData.RegionN]=OutData.Freq*2;
	}

	//Если новая точка больше верхнего диапазона графика
	if(MaxSeriesValue[OutData.RegionN]<OutData.Freq)
	{
		MaxSeriesValue[OutData.RegionN]=OutData.Freq;
		DxpsOutList::iterator i;
		int j=0;
		for (i = CDxpsRegion::OutData.begin(); i != CDxpsRegion::OutData.end();i++)
			if((i->RegionN==OutData.RegionN) && (j<LastSeriesIndex[OutData.RegionN]))
			{	//Перенос точек согласно новому масштабу
				SetElementYValue(j, OutData.RegionN,i->Freq*100/MaxSeriesValue[OutData.RegionN]);
				j++;
			}

	}

	if(MaxSeriesValue[OutData.RegionN]>=0)
	{
		PlotXY(OutData.Time/24./60./60., OutData.Freq*100/MaxSeriesValue[OutData.RegionN], OutData.RegionN);
	}
	else
	{
		PlotXY(OutData.Time/24./60./60., 0, OutData.RegionN);
	}
	SetElement(OutData.RegionN);
	if(CDxpsRegion::GetRegByN(OutData.RegionN)->Parameters.Type==TYPE_BE)
		str.Format("D%i: 100%% = %g Hz", OutData.RegionN+1, MaxSeriesValue[OutData.RegionN]);
	else
		str.Format("D%i: 100%% = %g", OutData.RegionN+1, MaxSeriesValue[OutData.RegionN]);
	SetElementName((LPCSTR)str);
	LastSeriesIndex[OutData.RegionN]++;
}
