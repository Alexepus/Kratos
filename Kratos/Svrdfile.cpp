...
BOOL SaveBinaryFile(FILE* fp)
{
	if(theApp.m_pMainFrame->m_Doc.m_DocType==CDoc::XPS||theApp.m_pMainFrame->m_Doc.m_DocType==CDoc::NoDoc)
	{
		...
		При изменении параметра региона переписывается этот регион.
		При изменении количества регионов переписывается весь файл.
		При прохождении одного прохода от начальной до конечной энергии региона
		переписываются данные всего прохода.

		*/
		CRegion* pReg;
		UINT ptrCurr = 0;
		unsigned char key[4];
		key[0]=0x01; key[1]=0x03; key[2]=0xbc; key[3]=0x13;
