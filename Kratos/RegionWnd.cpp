			...
			if (!RegNow) // RegEdit == FALSE
			{
				...
				if (m_pDlgParamReg->DoModal() == IDOK)
				{
					...
					if (oldDataIn != pReg->m_DataIn)
						pReg->m_DataIn.LastEditTime = time(nullptr); // ≈сли реально было изменение, то обновл€ем врем€ редактировани€
					...
					if (m_pMainFrame->m_Doc.IsFileOpen())
					{
						...
						m_pMainFrame->m_Doc.m_NeedSave = m_pMainFrame->m_Doc.Need;
						THRI_LOCK();
						if (ReWriteFile) // ѕерезапись всего файла если изменилс€ диапазон или шаг региона
							m_pMainFrame->m_Doc.SaveProjectFile();
						else //ѕерезаписать только параметры региона
