...
void CColourPicker::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
    ...
    CBrush* pOldBrush=(CBrush*) pDC->SelectObject(&WhiteBrush);
	CPen WhitePen(PS_SOLID,1,::GetSysColor(COLOR_WINDOW));
...
void CColourPicker::SetWindowSize()
{
    ...
    int nArrowHeight = rect.Height()-MarginSize.cy; //max(::GetSystemMetrics(SM_CYVTHUMB), 5*MarginSize.cy);
    CSize ArrowSize(nArrowWidth,nArrowHeight);//(max(nArrowWidth, nArrowHeight), max(nArrowWidth, nArrowHeight));
