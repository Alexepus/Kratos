...
void CItemData::InsertColumn(int nColumn)
{
	...
	aAllowEdit.InsertAt(nColumn, FALSE);
}
...
void CReportCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if(m_bEnableInput) _MouseClkMonitor(WM_LBUTTONDOWN, nFlags, point, TRUE);
}

void CReportCtrl::OnLButtonDblClk(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
	if(m_bEnableInput) _MouseClkMonitor(WM_LBUTTONDBLCLK, nFlags, point, TRUE);
}

void CReportCtrl::OnMButtonDown(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
	if(m_bEnableInput) _MouseClkMonitor(WM_MBUTTONDOWN, nFlags, point, FALSE);
}

void CReportCtrl::OnMButtonDblClk(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
	if(m_bEnableInput) _MouseClkMonitor(WM_MBUTTONDBLCLK, nFlags, point, FALSE);
}

void CReportCtrl::OnRButtonDown(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
	if(m_bEnableInput) _MouseClkMonitor(WM_RBUTTONDOWN, nFlags, point, FALSE);
}

void CReportCtrl::OnRButtonDblClk(UINT nFlags, CPoint point) 
{
	// TODO: Add your message handler code here and/or call default
	if(m_bEnableInput) _MouseClkMonitor(WM_RBUTTONDBLCLK, nFlags, point, FALSE);
}
...
//if Row=-1 or Column=-1, whole column or row or whole list become set to value
void CReportCtrl::SetEditable(BOOL bSet, int Row, int Column)
{
	CItemData* p;
	int i,j;
	if (!bSet)
	...
	if(Row==-1) //Set bSet for all rows
	{
		if(Column!=-1)
			aAllowEditForNewItems[Column]=bSet;
		else
		{
			for(i=0; i<GetColumnCount(); i++)
			aAllowEditForNewItems[i]=bSet;
		}

		for(i=0; i<GetItemCount(); i++)
		{
			p= (CItemData*)(CListCtrl::GetItemData(i));
			if(Column==-1) //Set bSet for whole list
			{
				for(j=0; j<p->aAllowEdit.GetSize(); j++)
				{
					p->aAllowEdit[j] = bSet;
				}
			}
	...
	{
		p= (CItemData*)(CListCtrl::GetItemData(Row));
		if(Column==-1) //Set bSet for all columns, but not all rows
		{
			for(j=0; j<p->aAllowEdit.GetSize(); j++)
			{
				p->aAllowEdit[j] = bSet;
				//aAllowEditForNewItems[i]=bSet; -- not all rows!
			}
...
BOOL CReportCtrl::IsEditable(int Row, int Column) const
{
	CItemData* p= (CItemData*)(CListCtrl::GetItemData(Row));
	if(p && Column<GetColumnCount())
		return p->aAllowEdit[Column];
	return FALSE;
}
...
void CReportCtrl::_MouseClkMonitor(UINT nMsg, UINT nFlags, CPoint point, BOOL bTriggerEdit)
{
	...
	if((hti.iItem>=0)&&(hti.iItem<GetItemCount()))
	{
		CItemData* p = (CItemData*)(CListCtrl::GetItemData(hti.iItem));
			if(p) 
			if (bTriggerEdit && p->aAllowEdit[hti.iSubItem] && !STATSCHANGED && !WAS_EDIT && WASACTIVE)
...
BOOL CReportCtrl::StartEdit(int nItem, int nSubItem)
{
	...
	if (!p->aAllowEdit[nSubItem] || !_IsValidIndex(nItem) || nSubItem < 0 || nSubItem >= GetColumnCount())
		return FALSE;
...
void CReportCtrl::_UpdateColumn(int nColumn, BOOL bInsert)
{
	...
	if (bInsert)
		aAllowEditForNewItems.InsertAt(nColumn, FALSE);
	else
		aAllowEditForNewItems.RemoveAt(nColumn);
...
int CReportCtrl::_CompareItems(int nItem1, int nItem2)
{
	...
	//move empty items down in ascending
...
void CReportCtrl::EnableInput(bool Enable)
{
	m_bEnableInput=Enable;
	for(int i=0; i<GetItemCount();i++)
	{
		SetItemStates(i,RC_ITEM_UNSELECTED);
	}

}

bool CReportCtrl::GetEnableInput()
{
