...

   {
	   ...
	   //dcb.fOutxDsrFlow=TRUE; 
		...
		if (!SetCommState(hCommDev, &dcb))
			AfxMessageBox("Ошибка при установке конфигурации COM порта.  ", MB_OK|MB_ICONSTOP);
...
if (!SetTimeoutsResult)
   ...
   AfxMessageBox("Ошибка при установке тайм-аутов COM порта.  ", MB_OK|MB_ICONSTOP);
...
int DetectSerialErrors(HANDLE hComm, CString *Err)
{
	...
	{
		AfxMessageBox("Report error in ClearCommError.  ", MB_OK|MB_ICONSTOP);
...
*/
	...
	*Err=str;
	...
	{
		CString str;
		str.Format("DetectSerialErrors: %s", str);
		AfxMessageBox((LPCSTR)str,  MB_OK|MB_ICONSTOP);
